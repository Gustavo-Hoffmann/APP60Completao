...
export function knowledgeBaseRouter(pool: Pool) {
  const r = Router();
  ...
  r.post("/", async (req, res) => {
    ...
    try {
      const ins = await pool.query(
        `INSERT INTO knowledge_base_items (kind, acronym, title, url, created_by_user_id)
         ...
         RETURNING id,
                   ...
                   url,
                   created_at`,
        [b.kind, b.acronym.trim(), b.title.trim(), b.url.trim(), u.id]
      );
