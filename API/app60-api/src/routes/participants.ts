    ...
    try {
      ...
      if (isSuperAdmin(u)) {
        const q = await pool.query(
          `SELECT p.id, p.cpf_normalized AS cpf, p.full_name, p.birth_date, p.sex,
        ...
        const inst = institutionIdOrThrow(u);
        const q = await pool.query(
          `SELECT DISTINCT p.id, p.cpf_normalized AS cpf, p.full_name, p.birth_date, p.sex,
                  p.city, p.state, p.created_at, p.updated_at
           FROM participants p
    ...
    try {
      ...
      if (pid) {
        ...
        const existing = await client.query(
          `SELECT id FROM participants WHERE cpf_normalized = $1`,
          [cpf]
        );
        if (existing.rows[0]) {
