...
export function institutionsRouter(pool: Pool) {
  const r = Router();
  const duplicateInstitutionMessage =
    "Já existe uma instituição cadastrada com este nome e unidade.";

  function isInstitutionNameUnitConflict(error: unknown) {
    return (
      (error instanceof Error && error.message === duplicateInstitutionMessage) ||
      (typeof error === "object" &&
        error !== null &&
        "code" in error &&
  ...
  async function assertUniqueInstitutionNameUnit(
    name: string,
    ...
    const params: unknown[] = [normalizedName, normalizedUnit];
    ...
    try {
      if (canManageInstitutions(u)) {
        const q = await pool.query(
          `SELECT i.id, i.name, i.slug, i.acronym, i.unit, i.country, i.state_or_county, i.city,
  ...
  r.patch("/:id", async (req, res) => {
    ...
    try {
      ...
      const out = await pool.query(
        `SELECT id, name, slug, acronym, unit, country, state_or_county, city,
                postal_code, street, neighborhood, street_number, complement,
                is_active, created_at, updated_at
  ...
  r.delete("/:id", async (req, res) => {
    ...
    try {
      ...
      const links = await pool.query<{
        ...
        participant_links_count: number;
        ...
        `SELECT
           ...
           (SELECT COUNT(*)::int FROM supervision_edges WHERE institution_id = $1) AS supervision_count`,
        [id]
      );
